...
use std::{ env, io, process, process::Command, collections::HashMap, thread::sleep, time::Duration,  };
use json::JsonValue;
...
//pub fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let game = &args[4];
	let api_key = &args[5];
	let to_sleep_str = &args[6];
	let to_sleep: u64 = to_sleep_str.parse()?;
	
	let v1game = fetch(&format!("v1/games/{}", game))?;
	let parsed_v1game = json::parse(&v1game)?;
	
	let gameId = parsed_v1game["data"]["id"].as_str().unwrap();
	/*
	...
	*/
	...
	let ggd_raw = fetch(&format!("v2/GetGameData?gameId={}", gameId))?;
    let parsed_ggd = json::parse(&ggd_raw)?;
...
for i in 0..parsed_ggd["values"].len() {
	...
	let categories_raw = fetch(&format!("v1/games/{}/categories", gameId))?;
	let parsed_categories = json::parse(&categories_raw)?;
    ...
    let selection: i32 = input_text.trim().parse()?;
    //if modifier == "debug" {
		println!("You picked category: {}", cats[selection as usize].name);
	//}
    let selected_cat = &mut cats[selection as usize];
    
    //
    
    let variables_raw = fetch(&format!("v1/categories/{}/variables", selected_cat.id))?;
    //let variables2_raw = fetch(&format!("v1/games/{}/variables", game_abbreviation), "variables2");
    let parsed_variables = json::parse(&variables_raw)?;
    ...
    println!("Which variables do you want to filter for? aka, runs with these variables and these options, will be uncredited: (answer format: \"1,3,5\" nothing for no variable filter)\n========");
    ...
    let mut filter_for_variables: bool = true;
    
    for selection in &selections {
		let thing: i32 = match selection.parse() {
				Ok(res) => res,
				Err(_) => { filter_for_variables = false; break }
			};
	...
	for run in il_runs {
		...
		if all_match {
			if run["category"].as_str() == Some(&selected_cat.id) {
	...
	println!("==================\nChange the speedruns? (yes or no)");
	let mut input_text = String::new();
    io::stdin().read_line(&mut input_text)?;
	...
	for run in filtered_il_runs {
		...
		let players_as_string2 = players_as_string.replace(
		...
		let response = ureq::put(&format!("https://www.speedrun.com/api/v1/runs/{}/players", run["id"]))
			.header("Content-Type", "application/json")
			.header("X-Api-Key", api_key)
			.send(&players_as_string2);
