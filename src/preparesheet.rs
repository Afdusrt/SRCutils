...
pub fn entry(args: &Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
	...
	let file_to_save = &args[3];
	let modifier = &args[args.len()-1];
	
	let ggd_raw = fetch(&format!("v2/GetGameData?gameUrl={}", game_abbreviation))?;
...
for i in 0..parsed_ggd["values"].len() {
	...
	let categories_raw = fetch(&format!("v1/games/{}/categories", game_abbreviation))?;
    ...
    //let variables2_raw = fetch(&format!("v1/games/{}/variables", game_abbreviation))?;
...
struct Category {
    ...
    let selected_plat = &plats[selection as usize];
    if modifier == "debug" {
    ...
    if input_text_trimmed.to_lowercase() == "no" {
		...
		selection = input_text_trimmed.parse()?;
	}
    let selected_region = &regions[selection as usize];
    if modifier == "debug" {
